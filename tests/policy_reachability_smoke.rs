...
#[test]
fn reports_unused_policy_class() {
    let request = std::fs::read(fixture_path(
        "test/fixtures/contracts/deadcode/policy-reachability.json",
    ))
    ...
    assert!(
        symbols.iter().any(|symbol| {
            symbol["symbol"] == "App\\Policies\\ReachablePolicyPostPolicy"
                && symbol["kind"] == "policy_class"
                && symbol["reachableFromRuntime"] == true
        }),
        "expected reachable policy class symbol, payload: {}",
        payload
    );

    assert!(
        findings.iter().any(|finding| {
            finding["symbol"] == "App\\Policies\\UnusedAuditPolicy"
                && finding["category"] == "unused_policy_class"
        }),
        "expected unused policy class finding, payload: {}",
        payload
    );
}
