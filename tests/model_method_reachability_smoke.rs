...
#[test]
fn reports_unused_model_method() {
    let request = std::fs::read(fixture_path(
        "test/fixtures/contracts/deadcode/model-methods.json",
    ))
    ...
    assert!(
        ...
        "expected reachable model method symbol, payload: {}",
        payload
    );
    ...
    assert!(
        ...
        "expected unused model method finding, payload: {}",
        payload
    );
}
