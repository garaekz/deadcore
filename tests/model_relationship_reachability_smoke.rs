...
fn reports_unused_model_relationship() {
    ...
    assert!(
        ...
        "expected unused model relationship finding, payload: {}",
        payload
    );
    ...
    assert!(
        !findings
            .iter()
            .any(|finding| finding["symbol"] == "App\\Models\\Invoice::customer"),
        "reachable model relationship should not be flagged as unused, payload: {}",
        payload
    );
}
